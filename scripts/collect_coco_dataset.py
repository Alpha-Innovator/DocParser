...
def extract_tex_files(path, target_pattern):
    ...
    for root, dirs, files in os.walk(path):
        for file in files:
            ...
            if not any(
                re.match(pattern, root.split("/")[-2]) for pattern in target_pattern
            ):
                continue
    ...
    for key, tex_files in dataset_dict.items():
        ...
        for tex_file in tqdm(tex_files):
            coco_annotation_file = (
                f"{os.path.dirname(tex_file)}/output/result/layout_annotation.json"
            )
            images_path = f"{os.path.dirname(tex_file)}/output/colored"

            if not re.search(pattern, tex_file):
            ...
            for idx in grouped_annotations_key_list:
                ...
                page_image = plt.imread(f"{images_path}/{idx}.png")
                ...
                shutil.copyfile(
                    f"{images_path}/{idx}.png", f"{target_images_folder}/{file_name}"
                )
